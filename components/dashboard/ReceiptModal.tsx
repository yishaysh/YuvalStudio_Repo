...
export const ReceiptModal: React.FC<ReceiptModalProps> = ({
    ...
    // Extract base service price
    let inferredBasePrice = 0;
    if (appointment.price && appointment.price > cartUndiscountedTotal) {
        inferredBasePrice = appointment.price - cartUndiscountedTotal;
    } else if (appointment.service_name && appointment.service_name !== 'כללי') {
        const matchingSrv = SERVICES.find(s => s.name === appointment.service_name);
        if (matchingSrv) inferredBasePrice = matchingSrv.price;
    }
    
    // Explicit service price defined? else fallback to inferred
    const mainServicePrice = (appointment.service_price !== undefined && appointment.service_price > 0) ? appointment.service_price : inferredBasePrice;
            ...
            <div className="flex flex-col gap-5">
                ...
                >
                    ...
                    <div className="text-center mb-8 mt-4 flex flex-col items-center">
                        <div className="w-auto h-20 mb-3 flex items-center justify-center relative">
                            <StudioLogo className="h-full w-auto text-brand-primary drop-shadow-[0_0_8px_rgba(212,181,133,0.3)]" />
                        </div>
