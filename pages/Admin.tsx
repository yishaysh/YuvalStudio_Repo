...
// 1. DASHBOARD TAB
const DashboardTab = ({ stats, appointments, onViewAppointment }: { stats: any, appointments: any[], onViewAppointment: (id: string) => void }) => {
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card>
            <h3 className="text-lg font-medium text-white mb-6">יעדי החודש</h3>
            <div className="space-y-6">
                <div>
                    <div className="flex justify-between text-sm mb-2">
                        <span className="text-slate-400">יעד הכנסות (20,000₪)</span>
                        <span className="text-brand-primary">{Math.round((stats.revenue / 20000) * 100)}%</span>
                    </div>
                    <div className="h-2 bg-brand-dark rounded-full overflow-hidden">
                        <motion.div 
                           initial={{ width: 0 }} 
                           animate={{ width: `${Math.min((stats.revenue / 20000) * 100, 100)}%` }}
